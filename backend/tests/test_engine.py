...
def rows(prices):
    return [
        {"date": f"2026-01-{i + 1:02d}", "open": p, "close": p}
...
def test_next_open_fees_and_final_liquidation():
    ...
    assert result["final_equity"] == pytest.approx(qty * 15 * 0.99 * 0.99, abs=0.01)
...
def test_no_future_leakage():
    ...
    assert (
        run_backtest(a, fast=2, slow=3)["curve"][:5]
        == run_backtest(b, fast=2, slow=3)["curve"][:5]
    )
